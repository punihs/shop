...
const {
  App, Auth, AccessToken, AuthCode, RefreshToken, Session,
} = require('../../conn/sqldb');

const log = debug('components/oauth');

const attributes = [
  'id', 'email', 'first_name', 'last_name', 'email', 'phone',
  'salutation',
];
...
const oAuthModel = {
  revokeToken(token) {
    ...
    return AccessToken
      ...
      .then((accessToken) => {
        ...
        const { session_id: sessionId, auth_id: userId } = accessToken;
        const expires = new Date();
        return Promise.all([
          AccessToken.update(
            { expires },
            { where: { auth_id: userId, session_id: sessionId } },
          ),
          RefreshToken.update(
            { expires },
            { where: { auth_id: userId, session_id: sessionId } },
  ...
  getAccessToken(bearerToken, callback) {
    ...
    return AccessToken
      .findOne({
        ...
        attributes: ['access_token', 'expires', 'session_id', 'app_id', 'auth_id'],
        raw: true,
      })
      .then((t) => {
        const token = t;
  ...
  getUserFromClient(clientId, clientSecret, cb) {
    ...
    return App
      .find(options)
      .then((client) => {
        if (!client || !client.Auth) return cb(null, false);
        cb(null, client.Auth.toJSON());
        return client;
      }).catch(cb);
  },
  ...
  getAuthCode(authCode, callback) {
    AuthCode
      .find({
        ...
        attributes: ['app_id', 'expires', ['auth_id', 'userId'], 'session_id', ['app_id', 'clientId']],
      })
      .then((authCodeModel) => {
  ...
  getRefreshToken(refreshToken, callback) {
    return RefreshToken
      .findOne({
        ...
        attributes: [['app_id', 'clientId'], ['auth_id', 'userId'], 'expires', 'session_id'],

        // only non suspended users should be allowed to refresh token
        include: [{ model: Auth, attributes: ['id'] }],
      })
