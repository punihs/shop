angular.module('uiGenApp')
  .controller('JobApplicantEditController', function JobApplicantEditController(QuarcService, Restangular, $state, moment, Upload, URLS, $stateParams, currentJob, currentApplicantToEdit, $filter, QCONFIG) {
    ...
    vm.data = currentApplicantToEdit;

    vm.Regions = {
      model: currentApplicantToEdit.region,
    ...
    vm.Degrees = {
      model: currentApplicantToEdit.degree_name,
    ...
    vm.Employers = {
      model: currentApplicantToEdit.employer_name,
    ...
    vm.Designations = {
      model: currentApplicantToEdit.designation_name,
    ...
    vm.uploadFile = function (jobId, applicantId, cvfile, payload, successCB, errorCB) {
      return vm.file = cvfile, cvfile ? (Upload.upload({
        url: URLS.API + '/jobs/' + jobId + '/applicants/' + applicantId,
        method: 'PUT',
        data: {
          fileUpload: cvfile,
      ...
      })) : Restangular.one('jobs', jobId).one('applicants', applicantId).customPUT(payload).then(function (response) { return successCB(response); }).catch(function (err) { errorCB(err); });
    };
    ...
    function checkApplicantBody(applicant) {
      /* Validating data found or not */
      let notFoundKey;

      const valueNotFoundFlag = keys.some(key => {
        notFoundKey = key;
        return typeof applicant[key] === 'undefined' || applicant[key] === '';
      });
    ...
    vm.create = function(jobId, applicantId, uploadCVForm) {
      ...
      const result = checkApplicantBody(data)
      if (result.flag) {
        return raiseError({
      ...
      return vm.uploadFile($stateParams.jobId, applicantId, vm.data.file, data, function jobCreated(response) {
        return setTimeout(function () {
          $state.go('applicant.view', { applicantId: applicantId });
        }, 1000);
      },
