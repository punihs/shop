angular.module('uiGenApp')
  .controller('ApplicantNewController', function NewJobCtrl(
    ...
    if (!currentJob) return $state.go('job.applicants.list', { jobId: $stateParams.jobId });

    if (currentJob.job_status === 'Closed' || currentJob.job_status === 'Hold') {
      return $state.go('job.applicants.list', { jobId: $stateParams.jobId });
    }
    ...
    vm.openApplicant = (id) => $window.parent.postMessage({ type: 'TOGGLE_PANEL', id }, '*');
    ...
    $window.onmessage = (e) => {
      ...
      vm.data.applicant_source_id = data.applicant_source_id || 9;
    ...
    vm.uploadFile = function (jobId, cvfile, payload, successCB, errorCB) {
      return vm.file = cvfile, cvfile ? (Upload.upload({
        url: URLS.API + '/jobs/' + jobId + '/applicants',
        data: {
          fileUpload: cvfile,
    ...
    vm.valdiateSummary = (uploadCVForm) => {
      const content = vm.data.ApplicantSummary.content;
    ...
    vm.create = function (newTab, uploadCVForm) {
      ...
      vm.uploadFile(
        ...
        (response) => {
          ...
          return $state.go('applicant.view', { applicantId: id });
        }, (err) => {
          vm.submitting = false;
